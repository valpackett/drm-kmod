...
void drm_gem_shmem_purge(struct drm_gem_shmem_object *shmem)
{
	...
	drm_vma_node_unmap(&obj->vma_node, obj /* XXX */);
#endif
...
static vm_fault_t drm_gem_shmem_fault(struct vm_fault *vmf)
{
	...
	if (page_offset >= num_pages ||
		...
		ret = lkpi_vmf_insert_pfn_prot_locked(vma, vmf->address, page_to_pfn(page), vma->vm_page_prot);
...
int drm_gem_shmem_mmap(struct drm_gem_shmem_object *shmem, struct vm_area_struct *vma)
{
	...
	if (obj->import_attach) {
		...
		// ret = dma_buf_mmap(obj->dma_buf, vma, 0); : XXX checks
		vma->vm_pgoff = 0;
		// vma_set_file(vma, obj->dma_buf->linux_file);
		ret = obj->dma_buf->ops->mmap(obj->dma_buf, vma);
